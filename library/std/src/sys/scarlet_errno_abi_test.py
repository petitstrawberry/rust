"""Check the native TLS implementation using the fork's production source.
...
RUST_SOURCE = Path(__file__).resolve().parents[4]
...
class NativeErrnoAbiTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        paths = (STD + "thread_local/key/scarlet.rs", STD + "thread/scarlet.rs",
                 STD + "pal/scarlet/os.rs", "library/scarlet-abi/src/tls.rs")
        cls.files = {name: (RUST_SOURCE / name).read_text() for name in paths}
    ...
    def test_production_helpers_share_preserve_and_isolate_errno(self):
        ...
        print(f"Native errno source: {RUST_SOURCE}", flush=True)
